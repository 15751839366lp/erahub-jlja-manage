 ...
 **/
@RestController
@RequestMapping("/asset/metadata/depreciationmethod")
@Validated
@Api(tags = "资产管理模块-元数据管理-折旧方法相关接口")
public class DepreciationMethodController {
    ...
    @RequiresPermissions({"asset:metadata:depreciationMethod:select"})
    ...
    @RequiresPermissions("asset:metadata:depreciationMethod:export")
...
//        String fileName = URLEncoder.encode("测试", "UTF-8").replaceAll("\\+", "%20");
    ...
    @RequiresPermissions({"asset:metadata:depreciationMethod:add"})
    @PostMapping("/addDepreciationMethod")
    public ResponseBean addDepreciationMethod(@RequestBody @Validated DepreciationMethodDTO depreciationMethodDTO) throws AssetException {
    ...
    public ResponseBean updateDepreciationMethod(@RequestBody @Validated DepreciationMethodDTO depreciationMethodDTO) throws AssetException {
    ...
    @RequiresPermissions({"asset:metadata:depreciationMethod:delete"})
    @ApiOperation(value = "删除折旧方法", notes = "删除折旧方法信息，根据折旧方法ID")
    @DeleteMapping("/deleteDepreciationMethod/{depreciationMethodId}")
    public ResponseBean deleteDepreciationMethod(@PathVariable Long depreciationMethodId) throws AssetException {
        depreciationMethodService.deleteDepreciationMethod(depreciationMethodId);
        return ResponseBean.success();
    }
    ...
    public ResponseBean deleteDepreciationMethodByBatchId(@RequestBody List<Long> depreciationMethodIds) throws AssetException {
    ...
    @RequiresPermissions({"asset:metadata:depreciationMethod:import"})
    @ApiOperation(value = "上传导入折旧方法", notes = "上传导入折旧方法")
    @PostMapping("/importDepreciationMethod")
    public ResponseBean importDepreciationMethod(HttpServletRequest request) throws AssetException, IOException {
