 ...
 **/
@RestController
@RequestMapping("/asset/metadata/section")
@Validated
@Api(tags = "资产管理模块-元数据管理-单位相关接口")
public class SectionController {
    ...
    @RequiresPermissions({"asset:metadata:section:select"})
    ...
    public ResponseBean<PageVO<SectionVO>> getChildrenList(@PathVariable String sectionId) throws AssetException {
    ...
    @RequiresPermissions("asset:metadata:section:export")
...
//        String fileName = URLEncoder.encode("测试", "UTF-8").replaceAll("\\+", "%20");
    ...
    @RequiresPermissions({"asset:metadata:section:add"})
    @PostMapping("/addSection")
    public ResponseBean addSection(@RequestBody @Validated SectionDTO sectionDTO) throws AssetException {
    ...
    public ResponseBean updateSection(@RequestBody @Validated SectionDTO sectionDTO) throws AssetException {
    ...
    @RequiresPermissions({"asset:metadata:section:delete"})
    @ApiOperation(value = "删除使用单位", notes = "删除使用单位信息，根据使用单位ID")
    @DeleteMapping("/deleteSection/{sectionId}")
    public ResponseBean deleteSection(@PathVariable String sectionId) throws AssetException {
    ...
    public ResponseBean deleteSectionByBatchId(@RequestBody List<String> sectionIds) throws AssetException {
    ...
    @RequiresPermissions({"asset:metadata:section:import"})
    @ApiOperation(value = "上传导入使用单位", notes = "上传导入使用单位")
    @PostMapping("/importSection")
    public ResponseBean importSection(HttpServletRequest request) throws AssetException, IOException {
